...
def _measurement_summary(measurement: Measurement) -> Dict[str, Any]:
    summary = {
        ...
        "status": "success",
        "detection_backend": measurement.detection.detector,
    ...
    if measurement.detection.detector_score is not None:
        summary["detection_score"] = measurement.detection.detector_score
    if measurement.detection.red_annulus_fraction is not None:
        summary["red_annulus_fraction"] = (
            measurement.detection.red_annulus_fraction
        )
    if measurement.detection.neutral_interior_fraction is not None:
        summary["neutral_interior_fraction"] = (
            measurement.detection.neutral_interior_fraction
        )
    return summary
...
def run(context: ExperimentContext) -> Dict[str, Any]:
    ...
    return {
        ...
        "preprocessing": {
            "2d_detector": str(
                detection_config.get("backend", "opencv_red_board")
            ),
        },
        "methods": {
            "2d": methods["2d"].name,
