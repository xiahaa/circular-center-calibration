...
Reference implementation and real-data sample for the paper **Accurate
Measurement of 3D and 2D Circular Centers With Application to LiDAR-Camera
Extrinsic Calibration**.

The repository contains these reusable algorithmic contributions:
...
It excludes large-scale evaluation infrastructure, Gazebo and ROS capture
pipelines, paper-figure generation, and historical prototypes.
...
data/                  Two Orbbec/Livox image-point-cloud datasets
```
...
- `orbbec_livox_office/`: 37 image/point-cloud pairs;
- `orbbec_livox_lab/`: 67 image/point-cloud pairs.

Both use `img/` and `pcd/` with matching numeric filenames and a 0.35 m
circular target. Images are raw distorted inputs; use the `camera_info.yaml`
...
AAMED source, or vendored third-party implementation is included. See
