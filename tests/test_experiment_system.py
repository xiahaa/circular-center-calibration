...
from experiments.qualitative_realworld.data_io import load_dataset, read_pcd  # noqa: E402
...
class ExperimentSystemTest(unittest.TestCase):
    ...
    def test_outer_config_stays_small_and_uses_paper_names(self):
        ...
        self.assertEqual(
            selection.datasets,
            ("orbbec_livox_lab", "orbbec_livox_office"),
        )
        ...
        insta_source = source.with_name("insta_hesai_outdoor.yaml")
        insta_selection = load_experiment_selection(insta_source)
        self.assertEqual(insta_selection.datasets, ("insta_hesai_outdoor",))
    ...
    def test_red_board_detector_returns_rectified_ellipse(self):
        ...
        config["backend"] = "opencv_red_board"
        ...
        self.assertEqual(detected.detector, "opencv_red_board")

    def test_aamed_detector_converts_and_selects_official_binding_output(self):
        image = np.full((480, 640, 3), 40, dtype=np.uint8)
        cv2.rectangle(image, (100, 60), (540, 420), (0, 0, 255), -1)
        cv2.ellipse(
            image,
        ...
        config = copy.deepcopy(config)
        config["backend"] = "aamed"
        config["aamed"]["module_search_paths"] = []

        class FakeAAMED:
            def __init__(self, rows, columns):
                self.shape = (rows, columns)

            def setParameters(self, theta, length, threshold):
                self.parameters = (theta, length, threshold)

            def run_AAMED(self, grayscale):
                self.input_shape = grayscale.shape
                # Official binding order: row, column, internal width/height,
                # internal angle, detector score.
                return np.array(
                    [[240.0, 320.0, 140.0, 180.0, -18.0, 0.95]],
                    dtype=np.float32,
                )
        ...
        fake_module = types.ModuleType("pyAAMED")
        fake_module.pyAAMED = FakeAAMED
        with mock.patch.dict(sys.modules, {"pyAAMED": fake_module}):
            detected = detect_target_ellipse(
                image, intrinsic, np.zeros(5), "plumb_bob", config
            )

        np.testing.assert_allclose(detected.ellipse[:2], [320.0, 240.0], atol=0.1)
        np.testing.assert_allclose(
            np.sort(detected.ellipse[2:4]), np.sort([70.0, 90.0]), atol=0.1
        )
        self.assertEqual(detected.detector, "aamed")
        self.assertAlmostEqual(detected.detector_score, 0.95, places=5)
        self.assertGreater(detected.red_annulus_fraction, 0.8)
    ...
    def test_binary_pcd_reader_accepts_hesai_vector_padding_fields(self):
        dtype = np.dtype(
            [
                ("x", "<f4"),
                ("y", "<f4"),
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            source = Path(temporary_directory) / "hesai.pcd"
            source.write_bytes(header + records.tobytes())
            cloud = read_pcd(source)
        np.testing.assert_array_equal(cloud.points, [[1, 2, 3], [4, 5, 6]])
        np.testing.assert_array_equal(cloud.intensity, [255, 180])

    def test_dataset_resolves_logical_name_from_manifest(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            data_root = Path(temporary_directory)
            root = data_root / "release_directory"
            (root / "img").mkdir(parents=True)
            (root / "pcd").mkdir()
            (root / "dataset.yaml").write_text(
                "dataset: insta_hesai_outdoor\n"
                "image_dir: img\n"
                "point_cloud_dir: pcd\n"
                "pair_count: 1\n"
            ...
            (root / "img" / "00000.png").touch()
            (root / "pcd" / "00000.pcd").touch()
            dataset = load_dataset(data_root, "insta_hesai_outdoor")

        self.assertEqual(dataset.name, "insta_hesai_outdoor")
