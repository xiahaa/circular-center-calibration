...
Each image and point-cloud pair has the same numeric filename. Verify the
released, privacy-masked 67-frame sequence before running it:
...
Verify the 37-pair office sequence in the same way:
...
The unmasked images are not needed for public reproduction. Their separate
hash manifest, `orbbec_livox_lab_raw_images.sha256`, exists only to identify the
author-side audit copy and must not be interpreted as public data.
...
Create the exact reference environment and install the working tree:
...
Alternatively, complete the root [Installation](../../../README.md#installation)
for a compatible, non-pinned development environment. Then run a 10-frame
preview:
...
Verify a complete run against the tracked full-precision result:
...
the precomputed-ellipse replay procedure.
...
Full-precision measurements are tracked in
`detector_comparison_seed2025.json`.
